...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile Channel12 -outputCreateChannelTx ./channel-artifacts/channel12.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile Channel12 -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors12.tx -channelID $CHANNEL_NAME -asOrg Org1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile Channel12 -outputAnchorPeersUpdate ./channel-artifacts/Org2MSPanchors12.tx -channelID $CHANNEL_NAME -asOrg Org2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile Channel12 -outputAnchorPeersUpdate ./channel-artifacts/Org3MSPanchors12.tx -channelID $CHANNEL_NAME -asOrg Org3MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "### Generating channel configuration transaction 'channel13.tx' ###"
  echo "#################################################################"
  set -x
  configtxgen -profile Channel13 -outputCreateChannelTx ./channel-artifacts/channel13.tx -channelID $CHANNEL_NAME1
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate channel configuration transaction..."
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for Org1MSP_1   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile Channel13 -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors13.tx -channelID $CHANNEL_NAME1 -asOrg Org1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Org1MSP_1..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for Org3_1MSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile Channel13 -outputAnchorPeersUpdate ./channel-artifacts/Org2MSPanchors13.tx -channelID $CHANNEL_NAME1 -asOrg Org2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Org3_1MSP..."
    exit 1
  fi
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile Channel13 -outputAnchorPeersUpdate ./channel-artifacts/Org4MSPanchors13.tx -channelID $CHANNEL_NAME1 -asOrg Org4MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
